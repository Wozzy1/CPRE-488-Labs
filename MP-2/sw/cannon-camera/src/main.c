...
static void Update_MultiBootRegister(void) __attribute__((unused));
/* Exception handlers */
...
void FsblFallback(void)
{
	...
	/*
	 * This trimmed FSBL only supports SD/MMC boot.
	 * Fallback image search for flash boot modes is intentionally disabled.
	 */
	ClearFSBLIn();
	FsblHookFallback();
	...
	/*
	 * Trimmed fallback path: simply hand control to the hook/reset flow.
	 */
