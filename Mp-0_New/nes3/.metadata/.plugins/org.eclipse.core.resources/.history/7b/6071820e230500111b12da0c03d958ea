...
#define HORIZONTAL_NEGATIVE_SPACE 64
...
void NESCore_Callback_OutputFrame(word *WorkFrame) {
	...
	for (i = 0 ; i < NES_DISP_HEIGHT; i++) {
		...
		for (j = 0; j < NES_DISP_WIDTH; j++) {
			...
			// for each pixel in the working frame, we want to output twice as big
			// in order to map 256x240 to 640x480


			for (h = 0; h < 2; h++) {
				for (w = 0; w < 2; w++) {
					ptr[(640 * (i*2+h)) + (HORIZONTAL_NEGATIVE_SPACE + j*2+w)] = tpixel;
				}
			}
	...
	// add black side bar so it looks cooler. "so it pops"
	for (i = 0; i < 480; i++) {
		for (j = 0; j < 64; j++) {
			ptr[640 * i + j] = 0;
			ptr[640 * i + (576+j)] = 0;
...
void NESCore_Callback_InputPadState(dword *pdwPad1, dword *pdwPad2) {
	...
	uint32_t buttons2 = Xil_In32(0xE000A00C);
...
//	#define NCTL_RIGHT  0x80
	...
	*pdwPad1 = 0x0;
	...
	if ((buttons & 0b10)) {
		xil_printf("Down pressed\n\r");
		*pdwPad1 |= NCTL_DOWN;
	}
	if ((buttons & 0b100)) {
		xil_printf("Left pressed\n\r");
		*pdwPad1 |= NCTL_LEFT;
	}
	if ((buttons & 0b1000)) {
		xil_printf("Right pressed\n\r");
		*pdwPad1 |= NCTL_RIGHT;
	}
	if ((buttons & 0b10000)) {
		xil_printf("Up pressed\n\r");
		*pdwPad1 |= NCTL_UP;
	}
	if(buttons2 == 4){
