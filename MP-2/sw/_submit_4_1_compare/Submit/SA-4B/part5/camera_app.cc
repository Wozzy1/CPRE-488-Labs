...
//	 YOUR Color Conversion SW algorithm will replace this SW pass-through code
	...
	for (g = 0; g < 1000; g++) {
		...
		for (i = 0; i < 1080; i+=2) {
			for (j = 0; j < 1920; j+=2) {
				...
				B = pS2MM_Mem[offset];
				G = pS2MM_Mem[offset + 1];
				R = pS2MM_Mem[offset + 1921];

				Y  = (R>>2) + (G>>1) + (B>>3);
				Cb = ((B-Y)>>1) + ((B-Y)>>2) + 128;
				Cr = ((R-Y)>>1) + ((R-Y)>>2) + 128;

				pMM2S_Mem[offset] = (Cb << 8) | Y;
				pMM2S_Mem[offset + 1] = (Cr << 8) | Y;
				pMM2S_Mem[offset + 1920] = (Cb << 8) | Y;
				pMM2S_Mem[offset + 1921] = (Cr << 8) | Y;
	...
	double seconds = (double)(timeEnd-timeStart)/(double)COUNTS_PER_SECOND;
	double fps = 1000/seconds;
